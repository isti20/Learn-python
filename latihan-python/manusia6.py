...
class Pegawai(ManusiaHidup):
    def __init__(self,nama,alamat,jamtidur):
        ...
        self.name = nama+ ' xxx'
        self.address = alamat+ ' yyy'
        self.jmlmata = 'dua'
        print(super().bersuara()) #Outputnya: haloo namaku wiku xxx (haloo namaku dari method bersuara di class ManusiaHidup, dan wiku xxx dari objek self.name di class pegawai)

dia = Pegawai('wiku','tasik','7')
print(dia) #Outputnya: dari method def __str__(self) di class ManusiaHidup tapi atributnya dari class Pegawai(ManusiaHidup)
